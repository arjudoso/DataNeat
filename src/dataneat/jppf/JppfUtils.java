...
public class JppfUtils {
	// create the JPPFClient. This constructor call causes JPPF to read the
	// configuration file
	 ...
	 * 
	 * @param jobName
	 *            an arbitrary, human-readable name given to the job.
	...
	public JPPFJob createJob(final String jobName) throws Exception {
		...
		// give this job a readable name that we can use to monitor and manage
		// it.
		job.setName(jobName);
	 ...
	 *             if an error occurs while executing the job.
	 */
	public void executeNonBlockingJob(final JPPFClient jppfClient, JPPFJob job) throws Exception {
		// Create a job
		...
		// the non-blocking job execution is asynchronous, we can do anything
		...
		// We use JPPFJob.awaitResults() for this. This method returns
		// immediately with
		// the results if the job has completed, otherwise it waits until the
		// job execution is complete.
	 ...
	 * "http://www.jppf.org/doc/v4/index.php?title=Submitting_multiple_jobs_concurrently">dedicated
	 * section</a> of the JPPF documentation for details and code samples.
	 * 
	 * @param jppfClient
	 ...
	 *             if any error occurs.
	 */
	public void executeMultipleConcurrentJobs(final JPPFClient jppfClient, List<JPPFJob> jobs) throws Exception {
		...
		// the non-blocking jobs are submitted asynchronously, we can do
	 ...
	 */
	private synchronized void processExecutionResults(final String jobName, final List<Task<?>> results) {
